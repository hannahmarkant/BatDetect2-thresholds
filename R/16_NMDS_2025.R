...
# grassland + drained peatland, PV on mineral soil, PV on rewetted peatland) in 2025.  
...
# The resulting stress value (~0.03) indicates a reliable two-dimensional 
...
pivot_tab <- audio_data_2025_filtered %>%
...
# Stress test for different dimensions (k) 
...
clim_vars <- nmds.sites %>% select(Rain_fall_month, Temperature_mean_month) 
...
clim_vars <- nmds.sites %>% select( Rain_fall_month, Temperature_mean_month, landuse)
...
cor_results <- data.frame(
  Variable = c(  "Rain_fall_month", "Temperature_mean_month", "landuse"), 
  NMDS1_rho = NA,
  NMDS1_p = NA,
  NMDS2_rho = NA,
  NMDS2_p = NA
...
NMDS_plot_combined <- ggplot(nmds.sites, aes(NMDS1, NMDS2)) +
  ...
  geom_text_repel(
    ...
    color = "black",
    size = 4,
    nudge_x = 0.02,    # optional kleiner Grundversatz
    nudge_y = 0.02,
    segment.color = NA  # keine Linie vom Label zum Punkt, optional
  )+
  theme_bw() +
  labs(color = "Landuse",
       caption = paste("Stress:", round(nmds1$stress, 3)))+
